...
namespace jwt {
	...
	namespace algorithm {
		...
		struct rsa {
			 ...
			 * \param name Name of the algorithm
			 */
			rsa(const std::string&, const std::string& private_key, const std::string&, const std::string& private_key_password, const EVP_MD*(*md)(), const std::string& name)
				: md(md), alg_name(name)
			{
			    // The payload is signed with private key - changed based on requirement for virtru.
                ...
                RSA* rsa = PEM_read_bio_RSAPrivateKey(privkey_bio.get(), nullptr, nullptr, (void*)private_key_password.c_str());
                EVP_PKEY_assign_RSA(priKey, rsa);
                pkey.reset(priKey, EVP_PKEY_free);
